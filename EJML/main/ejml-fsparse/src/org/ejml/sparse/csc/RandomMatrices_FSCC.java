...
er      * @param minFill minimum fill fraction
     ...
     * @return Random matrix
     */
    public static FMatrixSparseCSC symmetricPosDef( int width , int nz_total , Random rand ) {
        FMatrixSparseCSC A = rectangle(width,width,nz_total,rand);

        // to ensure it's SPD assign non-zero values to all the diagonal elements
        for (int i = 0; i < width; i++) {
            A.set(i,i,Math.max(0.5f,rand.nextFloat()));
        }

        FMatrixSparseCSC spd = new FMatrixSparseCSC(width,width,0);
        CommonOps_FSCC.multTransB(A,A,spd,null,null);
